...
Game::isCheckMate()
{
  ...
  const pair<const ChessPiece*, int> pair =
    oneAttackingPiece(oppTeam->getKing());
  ...
  if (count == 1) {
    // 3) It is not possible to capture the attacking piece (impossible if there
    // is more than one attacking piece).
    if (canCaptureAttacking(attackingPiece)) {
      return false;
    }

    // 4) It is not possible to interpose a piece between the king and the
    // attacking piece (impossible if there is more than one attacking piece,
    // impossible if the attacking piece is a knight).
    if (canBlockCheck(attackingPiece, oppTeam->getKing()) &&
        attackingPiece->getType() != ChessType::KNIGHT) {
      return false;
    }
  }
...
pair<const ChessPiece*, int>
Game::oneAttackingPiece(const ChessPiece* king)
{
  int count = 0;
...
bool
Game::canCaptureAttacking(const ChessPiece* attackingPiece)
{
  Team* attackedTeam =
    attackingPiece->getColor() == Color::Black ? &white : &black;
...
bool
Game::canBlockCheck(const ChessPiece* attackingPiece, const ChessPiece* king)
{
  ...
  for (const auto blockablePosition : blockablePositions) {
    for (const auto friendly : kingTeam->getPieces()) {
      if (friendly->getType() != ChessType::KING &&
          friendly->canMove(blockablePosition, b)) {
        return true;
      }
    }
  }
...
bool
Game::knightAttackingKing(const ChessPiece* king)
{
  Team* enemyTeam = king->getColor() == Color::White ? &black : &white;
  for (const auto enemy : enemyTeam->getPieces()) {
    if (enemy->getType() == ChessType::KNIGHT &&
        enemy->canMove(king->getPosition(), b)) {
      return true;
    }
  }
  return false;
...
Game::move(std::pair<int, int> start, std::pair<int, int> dest)
{
  ...
  } else if (b.getChessPiece(start.first, start.second)->getType() ==
               ChessType::PAWN &&
             dest.second != start.second) { // en passant capturing
    if (turnColor == Color::White) {
  ...
  if (b.getChessPiece(start.first, start.second)->getType() ==
      ChessType::KING) { // check for castling
  ...
  if (b.getChessPiece(start.first, start.second)->getType() ==
        ChessType::PAWN &&
      (dest.first == 0 || dest.first == 7)) { // promotion
    char t;
    if (turnColor == Color::Black &&
        black.getPlayer()->getType() == PlayerType::C) {
      t = black.getPlayer()->getPromotionChoice();
    } else if (turnColor == Color::White &&
               white.getPlayer()->getType() == PlayerType::C) {
