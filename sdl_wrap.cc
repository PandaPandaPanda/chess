...
Screen::Screen(int w, int h, std::string screenName)
  : window{ SDL_CreateWindow(screenName.c_str(),
...
void
Screen::draw_rect(int xloc, int yloc, int w, int h, Colour c)
{
...
void
Screen::add_font(string key, string path, unsigned int point)
{
...
void
Screen::draw_string(string msg, int x, int y, Colour c, string fontKey)
{
  SDL_Surface* textSurface =
    TTF_RenderText_Solid(fonts[fontKey], msg.c_str(), c);
  SDL_Texture* texture = SDL_CreateTextureFromSurface(render, textSurface);
  if (texture == NULL) {
    cerr << "Error loading string texture: " << SDL_GetError() << endl;
  }
  msgs.emplace_back(
    new OwningText{ x, y, textSurface->w, textSurface->h, texture });
  SDL_FreeSurface(textSurface);
}

void
Screen::add_img(string key, string path)
{
  SDL_Surface* imgSurface = IMG_Load(path.c_str());
  if (imgSurface == NULL) {
    cerr << "Error loading image at path: " << path << " - " << IMG_GetError()
         << endl;
    return;
  }
  SDL_Texture* texture = SDL_CreateTextureFromSurface(render, imgSurface);
  if (texture == NULL) {
    cerr << "Unable to create texture from image with path " << path << " - "
         << SDL_GetError();
    return;
  }
  SDL_FreeSurface(imgSurface);
  if (imgs.find(key) != imgs.end())
    SDL_DestroyTexture(imgs[key]);
  imgs[key] = texture;
}

void
Screen::draw_img(std::string key, int x, int y)
{
  int w, h;
